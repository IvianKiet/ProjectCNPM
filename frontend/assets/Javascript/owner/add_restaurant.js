...
// Add restaurant - THIS IS THE CRITICAL FUNCTION
btnAdd.addEventListener("click", async (e) => {
    ...
    console.log("Form values:", { name, address, province, phone, manager }); // Debug log
    ...
    if (!address || address.trim() === "") {
        ...
        validationErrors.push("Address is empty");
        isValid = false;
    }
    
    if (!province || province === "") {
        validationErrors.push("Province not selected");
        isValid = false;
    }
    
    const phoneRegex = /^[0-9]{10,11}$/;
    if (!phone || !phoneRegex.test(phone)) {
        document.getElementById("message_phone").textContent = "Số điện thoại không hợp lệ (10-11 số)";
        validationErrors.push(`Phone invalid: "${phone}"`);
        isValid = false;
    }
    
    if (!manager || manager.trim() === "") {
        document.getElementById("message_manager").textContent = "Vui lòng nhập tên quản lý";
        validationErrors.push("Manager name is empty");
        isValid = false;
    }
    ...
    console.log("Validation passed, creating branch..."); // Debug log
...
window.addEventListener('DOMContentLoaded', async () => {
    console.log("Page loaded, loading restaurants..."); // Debug log
